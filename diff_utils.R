...
diff_expr_one <- function (eset, gse_name, data_dir) {
    #INPUT:
    #OUTPUT:
    gse_dir <- paste(data_dir, gse_name, sep="/")

    #select contrasts
    cons <- add_contrasts(eset, gse_name)
...
add_contrasts <- function (eset, gse_name) {
    ...
    while (TRUE) {
        #select AL samples
        AL <- tk_select.list(choices, multiple=T, title="Control samples for contrast")
        AL <- str_extract(AL, "GSM[0-9]+")
        if (length(AL) == 0) {break}

        #select CR samples
        CR <- tk_select.list(choices, multiple=T, title="Test samples for contrast")
        ...
        pData(eset)[c(AL, CR), "tissue"] <- tissue
        ...
        mama_samples[[contrast_name]] <- c(AL, CR)

        #add labels for contrast to mama_clinicals
        labels <- factor(pData(eset)[c(AL, CR), "diet"], levels = c("AL", "CR"))   #  (ref, test)
        clinical <- data.frame(diet = labels, row.names = c(AL, CR))
...
diff_setup <- function(eset, group_levels){
    ...
    mod <- model.matrix(~0+group, data=pData(eset))
...
diff_anal <- function(eset, contrasts, group_levels, mama_data,
                      mod, modsv, svobj, gse_dir, gse_name){
